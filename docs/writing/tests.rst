...
`Unittest <https://docs.python.org/3/library/unittest.html#module-unittest>`_ — заряженный (навороченный) модуль тестирования в стандартной библиотеке Python. Его API знаком каждому, кто использовал любой из JUnit/nUnit/CppUnit серии инструментов.

Создание тестовых примеров выполняется путем подкласса: `unittest.TestCase <https://docs.python.org/3/library/unittest.html#unittest.TestCase>`_.
...
Модуль `doctest <https://docs.python.org/3/library/doctest.html#module-doctest>`_ ищет фрагменты текста, которые выглядят как интерактивные сеансы Python в docstrings, а затем выполняет эти сеансы, чтобы убедиться, что они работают точно так, как показано на рисунке. 
